...
def add_repo():
    ...
    try:
        ...
        return jsonify({"message": f"Repository {repo.name} added!", "repo_id": repo.id}), 201
...
def track_repo():
    if request.method == 'POST':
        ...
        try:
            ...
            response = requests.post(f"{BASE_API_URL}/repos", json={"repo_url": repo_url})
            print(f"Response from /repos: {response.status_code} - {response.json()}")  # Debug log
            ...
            if response.status_code == 201:
                ...
                if metrics_response.status_code == 200:
                    ...
                    repo_metrics = next((m for m in metrics if m['repo_url'] == repo_url), None)
                    if repo_metrics:
                        return render_template('results.html', repo_url=repo_url, metrics=[repo_metrics])
                    else:
...
def show_metrics(repo_id):
    ...
    if metrics_response.status_code == 200:
        ...
        if repo_metrics:
            return render_template('results.html', repo_url=repo.url, metrics=[repo_metrics])
        else:
